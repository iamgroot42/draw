...
enc_size = 128 # number of hidden units / output size in LSTM
dec_size = 128
...
def attn_window(scope,h_dec,N):
	with tf.variable_scope(scope,reuse=DO_SHARE):
		...
		params=linear(h_dec,7)
		# gx_,gy_,log_sigma2,log_delta,log_gamma=tf.split(params,5,1)
		gx_,gy_,log_sigma2,log_delta,log_gamma, scalea, scaleb=tf.split(params,7,1)
		# gx=(A+1)/2*(gx_+1)
		gx=scalea * (gx_+1)
		# gy=(B+1)/2*(gy_+1)
		gy=scaleb * (gy_+1)
...
rootDir = os.getcwd() + '/edges/edges2handbags/train/'
images = sorted(os.listdir(rootDir))

for i in range(train_iters):
	#xtrain,_=train_data.next_batch(batch_size) # xtrain is (batch_size x img_size)

	# xtrain = X_train[i * batch_size: (i+1) * batch_size]
	xtrain = [cv2.imread(rootDir + file, 0) for file in images[i1 * batch_size : (i1 + 1) * batch_size]]
	if (len(xtrain) != batch_size):
		temp = [cv2.imread(rootDir + file, 0) for file in images[:batch_size - len(xtrain)]]
		xtrain.extend(temp)
	# import pdb; pdb.set_trace()
	xtrain = [xtrain[i2][:, : xtrain[i2].shape[1] / 2] for i2 in xrange(len(xtrain))]
	xtrain = [cv2.resize(xtrain[i2], (xtrain[i2].shape[0] / 4, xtrain[i2].shape[1] / 4)).flatten() for i2 in xrange(len(xtrain))]
	i1 += 1
	feed_dict={x:xtrain}
